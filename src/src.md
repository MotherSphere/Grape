...
- `config.rs`
  - Préférences utilisateur (thèmes, accents, densité, accessibilité, audio avancé, stockage).
  - Chargement/sauvegarde JSON dans `~/.config/grape/preferences.json` (+ history/logs).
- `library.rs`
...
- `ui/`
  ...
  - Composants : `ArtistsPanel`, `AlbumsGrid`, `GenresPanel`, `FoldersPanel`,
    `SongsPanel`, `PlayerBar`, `PlaylistView`, `AudioSettings`.
