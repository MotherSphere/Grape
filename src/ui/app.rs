...
impl GrapeApp {
    ...
    fn album_entry_by_id_mut(
        &mut self,
        album_id: usize,
    ) -> Option<(&mut crate::library::Artist, &mut crate::library::Album)> {
        let mut id = 0usize;
    ...
    fn refresh_album_metadata_drafts(&mut self) {
        if let Some(selected_album) = self.ui.selection.selected_album.as_ref() {
            if let Some((_, album)) = self.album_entry_by_id(selected_album.id) {
                self.ui.album_genre_draft = album.genre.clone().unwrap_or_default();
                self.ui.album_year_draft = if album.year > 0 {
    ...
    fn handle_album_metadata_save(&mut self) {
        ...
        if let Some((_, album)) = self.album_entry_by_id_mut(selected_album.id) {
